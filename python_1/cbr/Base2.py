...
class Drops:
    ...
    emails = ['gposoportecoreip@izzi.mx', 'oscsalga@cisco.com', 'diefierr@cisco.com', 'mariomo@cisco.com',
              'jomarinm@cisco.com', 'imaganal@cisco.com', 'elrosale@cisco.com']
    ...
    def __init__(self, ip):
        try:
            self.net_connect = ConnectHandler(device_type='cisco_xr', ip=ip,
                                          username='collectorscp', password="CsPco11ecC0R3", timeout=20)
            self.ip = ip
    ...
    def salto(self, comando):
        self.net_connect.write_channel(comando)
        time.sleep(1)
        output = self.net_connect.read_channel()
        time.sleep(1)
        self.net_connect.write_channel("oscsalga" + '\n')
        time.sleep(1)
        self.net_connect.write_channel("devilmaycry2" + '\n')
        time.sleep(1)
        #output = self.net_connect.send_command("show running-config  | in hostname")
        #print(output)
